...
def technical_score(
    ...
    if not _is_finite(stoch_k) or not _is_finite(stoch_d):
        unavailable.append("Stoch")
    if not _is_finite(ma.get("dev200")):
        unavailable.append("200MA")

    if unavailable:
        return None, f"N/A: {', '.join(unavailable)}"
